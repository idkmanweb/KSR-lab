        ...
        public static async Task Main(string[] args)
        {
            ...
            var bus = Bus.Factory.CreateUsingRabbitMq(cfg =>
            {
                ...
                cfg.ReceiveEndpoint("klient_b_queue", ep =>
                {
                    ep.Handler<PytanieoPotwierdzenie>(async ctx =>
                    {
                        ...
                        if (key == ConsoleKey.T)
                        {
                            await ctx.Publish(new Potwierdzenie(ctx.Message.OrderId));
                            Console.WriteLine("\nWysłano potwierdzenie");
                        }
                        else
                        {
                            await ctx.Publish(new BrakPotwierdzenia(ctx.Message.OrderId));
            ...
            while (true)
            {
                if (!wTrakcieZamowienia)
                {
                    ...
                    {
                        var id = Guid.NewGuid();
                        string clientId = "KlientB";
                        await bus.Publish(new StartZamowienia(id, ilosc, new Uri("queue:klient_b_queue")));
